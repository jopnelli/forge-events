...
async function run() {
    const loopHoleVersion = await getLoopholeVersion().catch(() => {
        console.error("Loophole not found. The binary for this tool must be placed in the root of this project AND you have to be LOGGED IN. Download it from here: https://loophole.cloud/download")
    });
    if (!loopHoleVersion) {
        return;
    }
