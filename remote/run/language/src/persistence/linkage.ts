...
import {CollectionReference, DocumentReference, Transaction} from '@google-cloud/firestore';
...
export function languageLinkPersistence({authenticatedParentDoc}: { authenticatedParentDoc: DocumentReference }) {
  ...
  async function getRelatedLanguageLinks({
    transaction,
    linkRequestItems,
  }: { transaction: Transaction, linkRequestItems: LinkRequestItem[] }): Promise<LanguageLinkInFirestore[]> {
