...
import { linkRequestItemSchemas } from "../remote/run/events-app/src/schema/linkage";
import { createEvent, getLinks, putLinks } from "./persistence";
...
resolver.define("createEvent", async (req) => {
  // assertSchema(linkRequestItemSchemas, req.payload);
  return await createEvent(req.payload as EventInFirestore);
});
...
//     title =
//       VALID_LANGUAGES[
//         links.find((link) => link.pageId.toString() === id)!.languageISO2
//       ];
//     if (links.length > 1) {
//       title += ` (${links.length - 1} other language${
//         links.length > 2 ? "s" : ""
//       })`;
//     }
//   }
