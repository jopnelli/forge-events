...
export const AtlassianContext = React.createContext<AtlassianContextType>({loading: true, forgeContext: null});
type AtlassianContextType = { loading: boolean, forgeContext: ForgeContext | null };
 ...
 */
export const AtlassianContextProvider = ({children}: { children: ReactElement | ((params: AtlassianContextType) => ReactElement) }) => {
    ...
    return <>
      {[1, 2].map((_) =>
        <SkeletonItem
          key={_}
          isShimmering
          cssFn={(css) => ({'padding': 0, 'min-height': '30px', 'border-radius': '0'})}/>,
      )}
    </>;
  }
